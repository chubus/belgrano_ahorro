...
# -*- coding: utf-8 -*-
"""
Script para fusionar Belgrano Ahorro + Belgrano Tickets en una sola aplicación
"""
...
def fusionar_proyecto():
    """Fusionar todo el proyecto en una sola aplicación"""
    print("🔧 FUSIONANDO PROYECTO - BELGRANO AHORRO + TICKETS")
    print("=" * 60)
    
    # 1. Crear aplicación unificada
    print("\n1️⃣ Creando aplicación unificada...")
    crear_app_unificada()
    
    # 2. Copiar plantillas de la ticketera
    print("\n2️⃣ Copiando plantillas de la ticketera...")
    copiar_plantillas_ticketera()
    
    # 3. Actualizar base de datos
    print("\n3️⃣ Actualizando base de datos...")
    actualizar_base_datos()
    
    # 4. Crear archivos de configuración
    print("\n4️⃣ Creando archivos de configuración...")
    crear_archivos_configuracion()
    
    print("\n✅ PROYECTO FUSIONADO EXITOSAMENTE")
    print("=" * 40)
    ...
    print()
    print("🚀 Para iniciar:")
    print("   python app_unificado.py")

def crear_app_unificada():
...
'''
    ...
    with open('app_unificado.py', 'w', encoding='utf-8') as f:
        f.write(app_code)
    
    print("✅ Aplicación unificada creada")

def copiar_plantillas_ticketera():
    """Copiar plantillas de la ticketera"""
    try:
        # Crear directorio si no existe
        if not os.path.exists('templates/ticketera'):
            os.makedirs('templates/ticketera')
        ...
        for plantilla in plantillas:
            origen = f'belgrano_tickets/templates/{plantilla}'
            destino = f'templates/ticketera/{plantilla}'
            
            if os.path.exists(origen):
                shutil.copy2(origen, destino)
                print(f"   ✅ {plantilla}")
            else:
                print(f"   ⚠️ {plantilla} no encontrada")
                
    except Exception as e:
        print(f"❌ Error copiando plantillas: {e}")

def actualizar_base_datos():
    """Actualizar base de datos para incluir tickets"""
    try:
        # Verificar si la base de datos ya tiene la tabla de tickets
        import db as database
        
        # Intentar crear tabla de tickets si no existe
        try:
            database.crear_tabla_tickets()
            print("✅ Tabla de tickets creada")
        except:
            print("✅ Tabla de tickets ya existe")
            
    except Exception as e:
        print(f"⚠️ Error actualizando base de datos: {e}")

def crear_archivos_configuracion():
    """Crear archivos de configuración"""
    try:
        # Crear README de la aplicación unificada
        readme_content = '''# 🎉 BELGRANO AHORRO UNIFICADO
...
'''
        ...
        with open('README_UNIFICADO.md', 'w', encoding='utf-8') as f:
